...
export async function getEfdRegistries(fileList: FileList): Promise<[Analitico[], Analitico[]]> {
   ...
   const arr = getAnaliticoArr(analiticoMap['01-03-2023']);
   const [entradas, saidas] = partition(arr, c => +c.cfop < 4000);
   return [sort(entradas), sort(saidas)]
}

export async function createEfdSheet(entradas: Analitico[], saidas: Analitico[], link: HTMLAnchorElement) {
   const wb = getWb(entradas, saidas);
